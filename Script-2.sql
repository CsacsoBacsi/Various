...
update test_arr
  set attrs = array_append (attrs, '("X", "EUR", "2020-03-10 12:56:48")'::t_test)
  where id = 1 ; 

update test_arr
  set attrs = array_prepend ('("Y", "HUF", "2020-03-10 12:58:49")'::t_test, attrs) 
  where id = 1 ; 

update test_arr
  set attrs = attrs || ARRAY ['("C", "GBP", "2020-03-10 12:35:29")'::t_test] 
  where id = 1 ; 
...
CREATE OR REPLACE FUNCTION dt (IN p_schema text DEFAULT 'public', IN p_type text DEFAULT 'tp')
...
select * from dt ('public', '*') ; -- Gets tables, partitioned tables, views and materialized views
select * from dt ('public', 'rp') ; -- Default. Gets tables and partitioned tables
select * from dt ('public', 'r') ; -- Gets tables only
select * from dt ('public', 'p') ; -- Gets partitioned tables only
select * from dt ('public', 'v') ; -- Gets views only
select * from dt ('public', 'm') ; -- Gets materialized views only
...
CREATE OR REPLACE PROCEDURE dt2 (IN p_schema text default 'public', IN p_type text default 'rp')
LANGUAGE plpgsql
AS $$
DECLARE
query text ;
...
csr_ CURSOR (schema_name text) FOR
    SELECT n.nspname::text as "Schema",
	       c.relname::text as "Name",
	       CASE c.relkind WHEN 'r' THEN 'table' WHEN 'p' THEN 'partitioned table ' WHEN 'v' THEN 'view' WHEN 'm' THEN 'materialized view' END::text as "Type",
	       pg_catalog.pg_get_userbyid (c.relowner)::text as "Owner"
...
BEGIN
	...
	ELSIF p_type = 'rp' THEN
	    ...
	    CLOSE csr_tp ;
	ELSE
		OPEN csr_ (schema_name := p_schema) ;
	    
	    LOOP
	        FETCH csr_ into l_schema, l_name, l_type, l_owner ;
	        EXIT WHEN NOT FOUND ;
...
call dt2 ('public', '*') ; -- Gets tables, partitioned tables, views and materialized views
call dt2 ('public', 'rp') ; -- Default. Gets tables and partitioned tables
call dt2 ('public', 'r') ; -- Gets tables only
call dt2 ('public', 'p') ; -- Gets partitioned tables only
call dt2 ('public', 'v') ; -- Gets views only
call dt2 ('public', 'm') ; -- Gets materialized views only
